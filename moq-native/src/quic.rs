...
impl Endpoint {
	pub fn new(config: Config) -> anyhow::Result<Self> {
		...
		transport.max_idle_timeout(Some(time::Duration::from_secs(20).try_into().unwrap()));
		transport.keep_alive_interval(Some(time::Duration::from_secs(6))); // TODO make this smarter
