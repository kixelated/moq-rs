...
//!
//! The track is closed with [MoqError::Closed] when all publishers or subscribers are dropped.
...
impl State {
	...
	pub fn insert(&mut self, segment: segment::Subscriber) -> Result<(), MoqError> {
		self.closed.clone()?;

		let entry = match self.lookup.entry(segment.sequence) {
			indexmap::map::Entry::Occupied(_entry) => return Err(MoqError::Duplicate),
...
impl Publisher {
	...
	pub fn insert_segment(&mut self, segment: segment::Subscriber) -> Result<(), MoqError> {
		self.state.lock_mut().insert(segment)
	}

	/// Create an insert a segment with the given info.
	pub fn create_segment(&mut self, info: segment::Info) -> Result<segment::Publisher, MoqError> {
...
impl Subscriber {
	...
	/// Block until the next segment arrives, or return None if the track is [MoqError::Closed].
	pub async fn next_segment(&mut self) -> Result<Option<segment::Subscriber>, MoqError> {
		loop {
			let notify = {
